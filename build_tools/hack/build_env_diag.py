...
def where(exe:str):
    ...
    if exe_path is not None:
        VERSION_QUERY = subprocess.run(
            [f"{exe.lower()}", "--version"], capture_output=True, text=True, check=True).stdout.strip()
...
if WINDOWS:
    def get_regkey(root_key: RootKeys = "HKEY_LOCAL_MACHINE", 
                path:str=any, 
                key:str=any):
        """
        ...
        """
        from winreg import (
            HKEY_LOCAL_MACHINE, HKEY_CURRENT_USER, QueryValueEx, OpenKey
        )
        from typing import Literal
...
if WINDOWS:
    ...
    OS_MIN_VER = get_regkey("HKLM", r"SOFTWARE\Microsoft\Windows NT\CurrentVersion", "DisplayVersion")

    CPU_NAME = get_regkey("HKLM", r"HARDWARE\DESCRIPTION\System\CentralProcessor\0", "ProcessorNameString")
    ...
    for i in range(0, GPU_COUNT):
        _GPU_REG_KEY = str(r"SYSTEM\CurrentControlSet\Control\Class\{4d36e968-e325-11ce-bfc1-08002be10318}"+f"\\000{i}\\")
        GPU_CORE_NAME = get_regkey("HKLM", _GPU_REG_KEY, 'DriverDesc')
        
        if GPU_CORE_NAME != "Microsoft Basic Display Adapter":
            GPU_VRAM = get_regkey("HKLM", _GPU_REG_KEY, 'HardwareInformation.qwMemorySize')
            GPU_STATUS_LIST+= \
    ...
    DISK_USAGE_RATIO = float(DISK_USAGE_SPACE/DISK_TOTAL_SPACE)*100.
    ...
    LINUX_DISTRO_QUERY = subprocess.run(
        ["cat", "/etc/os-release"], capture_output=True, text=True, check=True).stdout
    LINUX_DIST_NAME    = re.search(r'^NAME="?(.*?)"?$', LINUX_DISTRO_QUERY, re.MULTILINE).group(1).strip()
    LINUX_DIST_VERSION = re.search(r'^VERSION_ID="?(.*?)"?$', LINUX_DISTRO_QUERY, re.MULTILINE).group(1).strip()
    LINUX_KERNEL_QUERY = subprocess.run(
        ["cat", "/proc/version"], check=True, capture_output=True, text=True).stdout
    LINUX_KERNEL_VALUE = re.search(r"Linux version (\d+)\.(\d+)\.(\d+)\.(\d+)", LINUX_KERNEL_QUERY)
    LINUX_KERNEL_MAJOR_VER, LINUX_KERNEL_MINOR_VER, _, _ = map(int, LINUX_KERNEL_VALUE.groups())
    WSL = True if "microsoft-standard-WSL2" in LINUX_KERNEL_QUERY.lower() else False

    CPU_LSCPU_QUERY = subprocess.run(
        ["lscpu"], capture_output=True, text=True, check=True).stdout
    CPU_ARCH = re.search(r"^\s*Architecture:\s*(.+)$", CPU_LSCPU_QUERY, re.MULTILINE).group(1).strip()
    CPU_NAME = re.search(r"^\s*Model name:\s*(.+)$", CPU_LSCPU_QUERY, re.MULTILINE).group(1).strip()
    CPU_CORE = os.cpu_count()

    MEM_INFO_QUERY = subprocess.run(
        ["cat", "/proc/meminfo"], capture_output=True, text=True, check=True).stdout
    MEM_PHYSIC_TOTAL = re.search(r"^MemTotal:\s+(\d+)\s+kB", MEM_INFO_QUERY, re.MULTILINE).group(1).strip()
    MEM_PHYSIC_AVAIL = re.search(r"^MemAvailable:\s+(\d+)\s+kB", MEM_INFO_QUERY, re.MULTILINE).group(1).strip()
    MEM_SWAP_TOTAL   = re.search(r"^SwapTotal:\s+(\d+)\s+kB", MEM_INFO_QUERY, re.MULTILINE).group(1).strip()

    DISK_STATUS_QUERY = subprocess.run(
        ["df", "-h", os.getcwd()], capture_output=True, check=True, text=True).stdout.strip().splitlines()[1].split()
...
if CPU_ARCH.upper() in ("AMD64", "X86_64", "X86-64", "X64", "Intel 64"):
    print(
    f"{Emoji_Pass} [CPU] CPU supported architecture: {CPU_ARCH}")
else:
    print(
    f"{Emoji_Error} [CPU] CPU unsupported architecture: {err(CPU_ARCH)}")
    printc(f"""
...
path = where(program)
if path:
    GIT_INSTALL_DIR, GIT_MAJOR_VERSION, GIT_MINOR_VERSION, GIT_PATCH_VERSION = where(program)
    print(
    f"{Emoji_Pass} [{program}] Found {program} {GIT_MAJOR_VERSION}.{GIT_MINOR_VERSION}.{GIT_PATCH_VERSION} at: {hint(GIT_INSTALL_DIR)}")
else:
    print(
    f"{Emoji_Error} [{program}] Cannot found {err(program)} program.")
    printc(f"""
...
path = where(program)
if path:
    GITLFS_INSTALL_DIR, GITLFS_MAJOR_VERSION, GITLFS_MINOR_VERSION, GITLFS_PATCH_VERSION = where(program)
    print(
    ...
    if WINDOWS:
        print(
        f"{Emoji_Warning} [{program}] Cannot found {warn(program)} program.")
        printc(f"""
...
if path:
    PYTHON_INSTALL_DIR, PYTHON_MAJOR_VERSION, PYTHON_MINOR_VERSION, PYTHON_PATCH_VERSION = where(program)
    if {PYTHON_MAJOR_VERSION} == 2:
        print(f"{Emoji_Error} [{program}] Found Python {PYTHON_INSTALL_DIR} is {err("Python 2")}.")
        printc(f"""
    ...
    """, Color_Error)
    elif (({PYTHON_MAJOR_VERSION} == 3) and ({PYTHON_MINOR_VERSION} < 8)):
            print(f"{Emoji_Warning} [{program}] Found Python {PYTHON_INSTALL_DIR} is version {warn(f"{PYTHON_MAJOR_VERSION}.{PYTHON_MINOR_VERSION}.{PYTHON_PATCH_VERSION}")}.")
            printc(f"""
    ...
    if "VIRTUAL_ENV" in ENV:
        print(
        f"{Emoji_Pass} [{program}] Found {program} {PYTHON_MAJOR_VERSION}.{PYTHON_MINOR_VERSION}.{PYTHON_PATCH_VERSION} at: {hint(PYTHON_INSTALL_DIR)} {hint("(Virtual ENV)")}")
    else:
        print(
        f"{Emoji_Warning} [{program}] Found {program} {PYTHON_MAJOR_VERSION}.{PYTHON_MINOR_VERSION}.{PYTHON_PATCH_VERSION} at: {warn(PYTHON_INSTALL_DIR)} {warn("(Global ENV)")}")
        printc(f"""
    ...
    print(
    f"{Emoji_Warning} [{program}] Cannot found {err(program)} program.")
    printc(f"""
...
if WINDOWS:
    ...
    path = where(program)
    if path:
        CHOCO_INSTALL_DIR, CHOCO_MAJOR_VERSION, CHOCO_MINOR_VERSION, CHOCO_PATCH_VERSION = where(program)
        print(
...
path = where(program)
if path:
    UV_INSTALL_DIR, UV_MAJOR_VERSION, UV_MINOR_VERSION, UV_PATCH_VERSION = where(program)
    print(
...
if WINDOWS:
    ...
    """, Color_Warning)
        ...
        print(
        f"{Emoji_Error} [{program}] {program} environment {err("not detected")}.")
        printc(f"""
    ...
    if ENV["VSCMD_ARG_HOST_ARCH"] != "x64":
        print(f"{Emoji_Error} [{VISUAL_STUDIO_VERSION}/@Host] Detected {VISUAL_STUDIO_VERSION} env host is {err(ENV["VSCMD_ARG_HOST_ARCH"])}.")
        printc(f"""
    We found your {VISUAL_STUDIO_VERSION} host is {ENV["VSCMD_ARG_HOST_ARCH"]}.
    ...
    """, Color_Error)
    else: print(f"{Emoji_Pass} [{VISUAL_STUDIO_VERSION}/@Host] Detected {VISUAL_STUDIO_VERSION} env host is {hint(ENV["VSCMD_ARG_HOST_ARCH"])}.")

    if ENV["VSCMD_ARG_TGT_ARCH"] != "x64":
        print(f"{Emoji_Warning} [{VISUAL_STUDIO_VERSION}/@Target] Detected {VISUAL_STUDIO_VERSION} env target {err(ENV["VSCMD_ARG_TGT_ARCH"])}.")
        printc(f"""
    We found your {VISUAL_STUDIO_VERSION} is targeting to {ENV["VSCMD_ARG_TGT_ARCH"]}.
    ...
    """, Color_Warning)
        ...
        print(f"{Emoji_Pass} [{VISUAL_STUDIO_VERSION}/@Target] Detected {VISUAL_STUDIO_VERSION} env targeting {hint(ENV["VSCMD_ARG_TGT_ARCH"])}.")

    if "VISUAL_STUDIO_VERSION" in globals():
        if "VCToolsInstallDir" and "VCToolsVersion" in ENV:
            print(
            f"{Emoji_Pass} [{VISUAL_STUDIO_VERSION}/MSVC] Detected MSVC version {hint(ENV["VCToolsVersion"])}."
            )
            program = "cl".lower()
            path = which(f"{ENV["VCToolsInstallDir"]}/bin/hostx64/x64/{program}.exe")
            if path:
                print(
                f"{Emoji_Pass} [{VISUAL_STUDIO_VERSION}/MSVC] Found MSVC C/C++ compiler driver at: {hint(path)}.")
            else:
                print(
    ...
    """, Color_Error)
            ...
            if path:
                print(
                f"{Emoji_Pass} [{VISUAL_STUDIO_VERSION}/MSVC] Found MSVC Macro Assembler at: {hint(path)}.")
            else:
                print(
    ...
    """, Color_Error)
            ...
            if path:
                print(
                f"{Emoji_Pass} [{VISUAL_STUDIO_VERSION}/MSVC] Found MSVC Linker at: {hint(path)}.")
            else:
                print(
    ...
    """, Color_Error)
            ...
            if path:
                print(
                f"{Emoji_Pass} [{VISUAL_STUDIO_VERSION}/MSVC] Found MSVC Archiver at: {hint(path)}.")
            else:
                print(
    ...
    """, Color_Error)
            ...
            if WIN_SDK_QUERY is not None:
                print(f"{Emoji_Pass} [{VISUAL_STUDIO_VERSION}/WindowsSDK] Found Windows SDK version {WIN_SDK_VERSION.group(1)}.")
            else:
                print(f"{Emoji_Error} [{VISUAL_STUDIO_VERSION}/WindowsSDK] No available Windows SDK installed.")
                printc(f"""
    ...
    """, Color_Error)
            ...
            print
            if path:
                print(f"{Emoji_Pass} [{VISUAL_STUDIO_VERSION}/WindowsSDK] Found Windows Resource Compiler at: {hint(path)}")
            else:
                print(f"{Emoji_Error} [{VISUAL_STUDIO_VERSION}/WindowsSDK] Cannot Find Windows Resource Compiler.")
                printc(f"""
...
if LINUX:
    ...
    path = where(program)
    if path:
        GCC_INSTALL_DIR, GCC_MAJOR_VERSION, GCC_MINOR_VERSION, GCC_PATCH_VERSION = where(program)
        print(
		f"{Emoji_Pass} [GCC/{program}] Found GCC Compiler {program} {GCC_MAJOR_VERSION}.{GCC_MINOR_VERSION}.{GCC_PATCH_VERSION} at: {hint(GCC_INSTALL_DIR)}.")
    else:
        print(
        f"{Emoji_Error} [GCC/{program}] Cannot found GCC Compiler {program}.")
        printc(f"""
    ...
    path = where(program)
    if path:
        GXX_INSTALL_DIR, GXX_MAJOR_VERSION, GXX_MINOR_VERSION, GXX_PATCH_VERSION = where(program)
        print(
        f"{Emoji_Pass} [GCC/{program}] Found GCC Compiler {program} {GXX_MAJOR_VERSION}.{GXX_MINOR_VERSION}.{GXX_PATCH_VERSION} at: {hint(GXX_INSTALL_DIR)}.")
    else:
        print(
        f"{Emoji_Error} [GCC/{program}] Cannot found GCC Compiler {program}.")
        printc(f"""
    ...
    if path:
        print(
		f"{Emoji_Pass} [GCC/{program}] Found GCC Assembler {program} at: {hint(path)}.")
    else:
        print(
        f"{Emoji_Error} [GCC/{program}] Cannot found GCC Assembler `{program}`.")
        printc(f"""
    ...
    if path:
        print(
		f"{Emoji_Pass} [GCC/{program}] Found GCC archiver {program} at: {hint(path)}")
    else:
        print(
        f"{Emoji_Error} [GCC/{program}] Cannot found GCC archiver `{program}`.")
        printc(f"""
    ...
    if path:
        print(
		f"{Emoji_Pass} [GCC/{program}] Found GCC linker {program} at: {hint(path)}")
    else:
        print(
        f"{Emoji_Error} [GCC/{program}] Cannot found GCC linker `{program}`.")
        printc(f"""
    We cannot found GNU Archiver `{program}`. Please check your compiler installation.
    If you not install it yet, please install it via your linux distro's package manager:
        sh $ apt install gcc g++
...
path = where(program)
if path:
    CMAKE_INSTALL_DIR, CMAKE_MAJOR_VERSION, CMAKE_MINOR_VERSION, CMAKE_PATCH_VERSION = where(program)
    ...
    print(
    f"{Emoji_Error} [{program}] Cannot found CMake program.")
    printc(f"""
...
path = where(program)
if path:
    NINJA_INSTALL_DIR, NINJA_MAJOR_VERSION, NINJA_MINOR_VERSION, NINJA_PATCH_VERSION = where(program)
    ...
    print(
    f"{Emoji_Error} [{program}] Cannot found Generator {program}.")
    printc(f"""
...
path = where(program)
if path:
    CCACHE_INSTALL_DIR, CCACHE_MAJOR_VERSION, CCACHE_MINOR_VERSION, CCACHE_PATCH_VERSION = where(program)
    ...
    print(
    f"{Emoji_Warning} [{program}] Cannot found Compiler cache '{program}'.")
    printc(f"""
...
if WINDOWS:
    MAX_PATH_LEN_STATUS = get_regkey("HKLM", r"SYSTEM\CurrentControlSet\Control\FileSystem", key="LongPathsEnabled")
...
if WINDOWS:
    ...
    if ((DISK_AVAIL_SPACE >=128) and (DISK_USAGE_RATIO < 80)):
        print(
        f"{Emoji_Pass} [DISK_SPACE] Disk space check passing.")
        printc(f"""
    ...
    """, Color_Hint)
        ...
        print(
            f"{Emoji_Warning} [DISK_SPACE] Disk space ckeck attention."
        )
        printc(f"""
