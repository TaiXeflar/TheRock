    ...
    def __init__(self, version_input):
        ...
        if isinstance(version_input, str):
            ...
            self.major, self.minor, self.patch, self.suffix = \
                version_input.major, version_input.minor, version_input.patch, version_input.suffix
        else:
            self.fullname = str(version_input)
...
def VERSION_BLACKLIST(v: str | VersionNum, 
...
def EXISTS(
    ...
    if file is not None:
        ...
        path_res = Path(f_str).as_posix() if os.path.exists(f_str) else Path(shutil.which(f_str)).resolve().as_posix()
