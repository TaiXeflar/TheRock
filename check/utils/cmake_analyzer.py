...
def cmake_variable_finder(
    ...
    if output == "all":
        ...
        if invalid_vars:
            raise ValueError(f"Error: variables in output is not allowed in hint range -> {invalid_vars}")
    ...
    def resolve_value(val: str) -> str:
        for _ in range(15): 
            if "${" not in val and "$ENV{" not in val:
                break
            val = re.sub(r'\$ENV\{([A-Za-z0-9_]+)\}', lambda m: os.environ.get(m.group(1), ""), val)
            val = re.sub(r'\$\{([A-Za-z0-9_]+)\}', lambda m: state.get(m.group(1), ""), val)
        return val
