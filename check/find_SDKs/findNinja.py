...
class FindNinja(FindSDK):
    ...
    def _post_ninja_avail(self, 
        ...
        MIN = VersionNum(min) if isinstance(min, (str, VersionNum)) else self.THEROCK_REQUIRED_NINJA
        ...
        if self.exe is NOTFOUND:
            ...
            result = SUCCESS if VERSION(self.Version, ">=", MIN, blacklist=blacklist, fuzzy=False) else self._compoment_fail(condition, fail_level)
        
        elif MIN and MAX:
            result = SUCCESS if VERSION_IN_RANGE(MIN, "<=", self.Version, "<=", MAX) else self._compoment_fail(condition, fail_level)
    ...
    def _post_ninja_maxjob(self,
        ...
        if limit > 0:
            ...
            message("FATAL_ERROR", f"Posting on MAX cores must higher than 0 or allow all cores by set to 0.")
