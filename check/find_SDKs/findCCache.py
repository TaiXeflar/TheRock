...
class FindCCache(FindSDK):
    ...
    def _parse_ccache_output(self):
        ...
        ccache_query = subprocess.run(cmd,
                                      capture_output=True,
                                      text=True,
                                      check=True).stdout.replace("\\", "/").replace("\"", "\'")
...
class FindSCCache(FindSDK):
    ...
    def __init__(self):
        ...
        self._dispatch_map = {
            "AVAIL": self._post_sccache_avail
        }
    ...
    def _find_sccache(self):
        ...
        if exe:
            query = subprocess.run(["sccache", "-s"], 
                                   capture_output=True, 
                                   text=True,  
                                   check=True).stdout.replace("\\", "/").replace("\"", "\'")
