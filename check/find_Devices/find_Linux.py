...
_wsl2_err = dedent(f"""Found Linux is WSL2 environment.
...
class FindLinux(FindSDK):
    ...
    @property
    def OS_TYPE(self):  return "Linux"
    
    @property
    def OS_NAME(self):  return self.info["OS_NAME"]

    @property
    def OS_VERSION(self):   return self.info["OS_VERSION"]

    @property
    def OS_KERNEL(self):    return self.info["OS_KERNEL"]

    @property
    def WSL2(self):     return self.info["WSL2"]

    @property
    def ENDIAN(self): return self.info["ENDIAN"]
    ...
    def _post_endian(self,
        ...
        if byteOrder == endian:
            ...
            reason = dedent(f"""Byte Order examination failed.
    ...
    def _post_network(self,
        ...
        if result is not SUCCESS:
            reason = dedent(f"""Found your device have no available internet connection.
    ...
    def __WINDOWS__(self):
        message("FATAL_ERROR", "Windows Device doesn't support Linux build mode!", RuntimeError)
