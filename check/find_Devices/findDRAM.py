...
class FindDRAM(FindSDK):
    ...
    def __init__(self):
        ...
        self.info: dict[
            Literal["MEM_PHYS_TOTAL", "MEM_PHYS_AVAIL", "MEM_VITURAL_AVAIL", "MEM_SWAP_AVAIL", "MEM_USE_RATIO", "MEM_ECC_LEVEL"],
            Any
        ]
    ...
    def __WINDOWS__(self) -> dict[str, Any]:
        ...
        return {"MEM_PHYS_TOTAL":       round(MEM_PHYS_TOTAL, 2),
    ...
    def _win_get_ecc(self):
        ...
        cmd = ["powershell", "-Command", "Get-CimInstance Win32_MemoryArray | Select-Object ErrorInfo | ConvertTo-Json"]
        cmd_output = subprocess.run(cmd, text=True, capture_output=True, check=True).stdout
        ...
        if ecc_info == 3 or ecc_info is None:
            ...
            message("FATAL_ERROR", f"ECC mem level read with value {ecc_info}", RuntimeError)
    ...
    def _linux_get_ecc(self):
        ...
        if is_sudo:
            ...
            try:
                output = subprocess.check_output(["lshw", "-C", "memory"], stderr=subprocess.DEVNULL).decode()
                if "ECC" in output:
                    return "ECC"
                return "UDIMM"
            except:
    ...
    def _post_dram_volume(self, 
        ...
        if result is not SUCCESS:
            reason = dedent(f"""Found System have low physical DRAM total volume.
    ...
    def _post_ecc_memory(self, 
        ...
        if memory is not None:
            if memory == "UDIMM":
                ...
                result = SUCCESS if self.MEM_ECC_LEVEL in ["ECC", "REG"] else self._compoment_fail(condition, fail_level)
            elif memory == "REG":
                result = SUCCESS if self.MEM_ECC_LEVEL == memory else self._compoment_fail(condition, fail_level)

        elif memory is None:
            result = SUCCESS
            
        else:
            message("FATAL_ERROR", f"Unsupported DRAM Type {memory}. Expected with UDIMM, ECC and REG.")

        message("CHECK", self.MEM_ECC_LEVEL, check_result=result)

        if result is not SUCCESS:
            reason = dedent(f"""Cannot confirm or Found unexpected DRAM Type.
