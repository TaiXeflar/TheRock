...
class FindCPU(FindSDK):
    ...
    required_cpu_arch = ["AMD64", "EM64T", "Intel64", "Intel 64", "x86_64", "x86-64", "x64"]

    def __init__(self):
        super().__init__()

        self.info: dict[
            Literal["CPU_NAME", "CPU_ARCH", "CPU_CORE"],
            Any]
    ...
    def __WINDOWS__(self):
        ...
        _cpu_name = regedit("HKLM", r"HARDWARE\DESCRIPTION\System\CentralProcessor\0",
    ...
    def _post_arch(self, 
        ...
        result = self._compoment_fail(condition, fail_level) if self.CPU_ARCH not in self.required_cpu_arch else SUCCESS
        message("CHECK", self.CPU_ARCH, check_result=result)

        if result is not SUCCESS:
            reason = dedent(f"""TheRock only supports AMD64/EM64T CPU targets.
    ...
    def _post_cores(self, 
        ...
        if result is not SUCCESS:
            reason = dedent(f"""TheRock would needs a powerful machine to compile.
